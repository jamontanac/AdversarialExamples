...
def init_model(lr:float=0.001,model_name:str="Resnet",classes:int=10)->Tuple[torch.nn.Module,torch.nn.Module,torch.optim.Optimizer,torch.optim.lr_scheduler.LRScheduler]:
...
def train(epoch:int,model:torch.nn.Module,optimizer:torch.optim.Optimizer, criterion:torch.nn.Module,trainloader:torch.utils.data.DataLoader)-> Tuple[float,float]:
    ...
    for i, data in enumerate(trainloader):
        ...
        if i % 100 == 99:
            ...
            # print(f"[{epoch + 0}, {i + 1}] loss: {loss.item():.3f}| Accuracy: {100*correct/total:.3f} ({correct} / {total})")
    acc= 100* correct/total
    logger.info(f"Total Accuracy: {acc}% over the {total} images") 
...
def Train_model(trainloader:torch.utils.data.DataLoader,testloader:torch.utils.data.DataLoader,parameters:Dict)->Tuple[torch.nn.Module,pd.DataFrame]:
    train_loss_hist = []
    test_loss_hist  = []
    train_acc_hist  = []
    test_acc_hist  = []
    ...
    model, criterion, optimizer, scheduler = init_model(model_name=name_model,classes=num_classes)
    best_acc = 0
